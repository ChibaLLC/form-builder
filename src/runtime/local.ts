      ...
      return h(
        ...
        h(Page, {
          ...
          onUpdateField: (index: number, field: FormElementData) => {
            const canvas = canvases[index];
            if (canvas) {
              canvases[index] = canvas.map((el) =>
                el.index === index ? field : el
              );
            } else {
              console.warn("Canvas Index not found");
            }
...
export class Elements {
  ...
  private events: { [key: string]: Array<(data: FormElementData) => void> } =
    {};
  private readonly FormElementsRenderer: ResolvedComponent;
  ...
  constructor(
    FormElementsRenderer: ResolvedComponent,
  ...
  on(
    ...
    callback: (data: Reactive<FormElementData>) => void
  ) {
  ...
  createComponent(data: Reactive<FormElementData>): Component {
    if (!isReactive(data)) {
      console.warn(
        "Non reactive data passed to create component of Elements: ",
        data
      );
    }
    return defineComponent({
      setup: () => {
        ...
        const activePageIndex =
          inject<Ref<number | undefined>>(activePageIndexKey);
        return {
      ...
      render() {
        return h(
          ...
          h(this.FormElementsRenderer, {
            data: data,
            active: computed(() => {
              const onCurrentPage =
                this.pageIndex === this.reactive.activePageIndex.value;
  ...
  delete(index: number) {
    ...
    if (!element) return console.warn("Wee jamaa!");
  ...
  setActive(state: boolean, data: Reactive<FormElementData>) {
    if (!isReactive(data)) {
      console.error(
        "Non reactive data passed to setActive of Elements: ",
        data
      );
    }
  ...
  move(oldIndex: number, newIndex: number) {
    ...
    const _old = this.components.get(oldIndex);
    const _new = this.components.get(newIndex);
    if (_new) {
      this.components.set(oldIndex, _new);
    }
    if (_old) {
      this.components.set(newIndex, _old);
    }
    return this;
  }
  ...
  repositionElement() {
    if (this.latestEnter === undefined)
      return console.warn("No element to reposition");
...
export function clearPages(): Promise<void> {
  return new Promise((resolve) => {
    if (import.meta.client) {
      ...
      window.dispatchEvent(new CustomEvent("nuxt-form-builder:clear-pages"));
      setTimeout(() => {
        resolve();
      });
    }
...
export function clearEmptyPages(): Promise<void> {
  return new Promise((resolve) => {
    window.dispatchEvent(
      new CustomEvent("nuxt-form-builder:clear-empty-pages")
    );
    setTimeout(() => {
      resolve();
    });
...
export function clearEmptyStores(): Promise<void> {
  return new Promise((resolve) => {
    window.dispatchEvent(
      new CustomEvent("nuxt-form-builder:clear-empty-stores")
    );
    setTimeout(() => {
      resolve();
    });
...
export async function clearEmptyCanvases(): Promise<void>{
  await Promise.all([clearEmptyStores(), clearEmptyPages()]);
  return;
}

export function createStorePage(
  Store: ReturnType<typeof resolveComponent>,
  stores: Stores,
  starter?: Item[] | undefined
) {
  return defineComponent({
    setup() {
