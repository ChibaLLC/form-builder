...
const props = defineProps({
  ...
  form: {
    type: Object as PropType<{ pages: Pages; stores: Stores }>,
    required: true,
  },
});
...
const fields = computed(() => {
  const fields = Object.values(props.form.pages);
  return fields.flat();
});

function setDependsOn(event: Event) {
  const target = event.target as HTMLSelectElement;
  if (target.value) {
    if (!props.active) return console.warn("Active property not found");
    props.active.dependsOn = {
      label: target.value,
      value: "",
      condition: "eq",
    };
  } else {
    if (!props.active?.dependsOn) return;
    props.active.dependsOn = undefined;
  }
}

const dependsOnField = computed(() => {
  if (!props.active || !props.active.dependsOn?.label) return undefined;
...
<template>
  <div class="properties-container" :style="styles">
    ...
    >
      <Transition name="fade">
        <TransitionGroup
          ...
          "
        >
          <div :key="active.index" class="properties">
            ...
            <ul>
              <li class="unset">
                <ul>
                  ...
                  <li class="rule">
                    <div>Depends on</div>
                    <select class="input" @change="setDependsOn">
                      <option
                        :value="''"
                        :selected="!active.dependsOn?.label"
                        class="option"
                      >
                        ------
                      </option>
                      <option
                        v-for="field of fields.filter(
                          (f) => f.label !== active?.label
                        )"
                        :value="field.label"
                        :selected="active.dependsOn?.label === field.label"
                      >
                        {{ field.label || "---" }}
                      </option>
                    </select>
                  </li>
                  <li v-if="active.dependsOn" class="rule">
                    <div>Show When</div>
                    <div>
                      <select
                        class="input"
                      ...
                      >
                        <option
                          value="eq"
                          :selected="active.dependsOn.condition === 'eq'"
                        >
                          =
                        </option>
                        <template v-if="isNumberField(dependsOnField)">
                          <option
                            value="lt"
                            :selected="active.dependsOn.condition === 'lt'"
                          >
                            <
                          </option>
                          <option
                            value="gt"
                            :selected="active.dependsOn.condition === 'gt'"
                          >
                            >
                          </option>
                        </template>
                      ...
                      <select
                        ...
                        "
                        v-if="isCheckbox(dependsOnField) || isSelect(dependsOnField)"
                        v-model="active.dependsOn.value"
                      >
                        <option
                          v-for="item of dependsOnField.options"
                          :value="item.value"
                        >
                          {{ item.label }}
                        </option>
                ...
                <ul style="margin-top: -5px">
                  <li v-for="item of active.options" class="unset item">
                    ...
                    <div
                      class="delete"
                      @click="
                        active.options = active.options?.filter(
          ...
          >
            ...
            <span style="display: flex"
              >Press the `Escape` key to deselect</span
            >
          </div>
        </TransitionGroup>
...
.properties-container ul > li:not(.unset) > * {
  display: flex;
  justify-content: space-between;
  align-items: center;
}
...
.properties-container li:not(.unset) > *:not(:has(> input)) {
  margin-top: 6px;
}
